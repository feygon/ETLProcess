...
namespace BasicPreprocess.General.Containers
{
    ...
    *************************************************************************************/
    ...
    /// </summary>
    /// <typeparam name="T1">Type of primary member of composite key</typeparam>
    /// <typeparam name="T2">Type of secondary member of composite key</typeparam>
    [Serializable]
    public class Auto_Incr_IntMap<T1, T2> : Dictionary<(T1 primary, T2 secondary), int>
    {
        ...
        public Auto_Incr_IntMap(List<(T1, T2)> compositeKeyColumnValues) : base()
        {
            foreach ((T1, T2) key in compositeKeyColumnValues)
            {
                Add(key);
            } // end loop
        } // end method
