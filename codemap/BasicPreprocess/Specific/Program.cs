...
{
    /// <summary>
    /// GPHealth's Statement file
    /// </summary>
    M691,
    /// <summary>
    /// GPHealth's Member File
    /// </summary>
    M504A,
    /// <summary>
    /// GPHealth's Balance Forward file
    /// </summary>
    M690,
    /// <summary>
...
namespace BasicPreprocess
{
    ...
    internal static class Program
    {
        ...
        private static GPHealthPreProcess gpHealth;

        static private (KeyedDocs<DocM691_Invoice>
        ...
        private static void Main(string[] args)
        {
            try
            {
                ...
                gpHealth = new GPHealthPreProcess(files);
                gpHealth.SetDebug(args[0]);
                
                gpHealth = new GPHealthPreProcess(files);

                // get files into temp directory

                CheckFiles(gpHealth.Files); // len check for implementation Statement/WelcomeLetters

                // input data to 691, 504A, 690
                docs = PopulateDocs(gpHealth.Files);

                // TO DO: process them separately by filtering business rules
                // TO DO: process them together by key-based business rules
                gpHealth.ProcessDocs(docs);
...
//            foreach (string filePath in gpHealth.Files)
//            {
...
//                using DataTable docGenWebAccounts = GetGPHealthAccounts.Execute();
...
//                    if (docGenWebAccounts.Select($@"MEMBERID = '{doc.memberID}' AND ACCOUNTID = '{doc.accountNumber}'").Length > 0)
//                    {
...
//                {
//                    var errorEmail = new ETLPEmail();
//                    string badOutFileName = $@"\\SERVER\SUBMIT\prt\GPHealth Bad 691 Records {DateTime.Now:yyyyMMdd}.csv";
//                    errorEmail.To.Add(@"Sample@ETLP.sample.com");
//                    errorEmail.Subject = @"GPHealth Processing Bad Records";
//                    errorEmail.Body = $"Hello,\r\nPlease upload the file located at {badOutFileName} to GPHealth's SFTP and notify them of the upload.\r\nThank You,\r\nGPHealth Statements PreProcessing";
