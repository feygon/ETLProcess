...
namespace BasicPreprocess.General.Containers
{
        ...
        : BasicDoc, iDocType_Takes_StringMap<TBasicDoc>, new()
    {
        ...
        Auto_Incr_IntMap<List<string>> uniqueInt_byComposite;
        ...
        private Dictionary<List<string>, TBasicDoc> compositeKeyedData = new Dictionary<List<string>, TBasicDoc>();
        private Dictionary<(string key, int i), TBasicDoc> primaryIndexKeyedData = new Dictionary<(string key, int i), TBasicDoc>();
        private Dictionary<(List<string> ckey, int i), TBasicDoc> compositeIndexKeyedData = new Dictionary<(List<string> ckey, int i), TBasicDoc>();
        KeyType keyType;
        ...
        public KeyedDocs(HeaderSource<List<StringMap>, List<string>> source)
        {
            ...
            // Construct a keyed data set from a header source, by its key type.
            switch (keyType)
            {
                ...
                case KeyType.compositeUnique:
                    ...
                    compositeKeyedData = new Dictionary<List<string>, TBasicDoc>();
                    foreach (StringMap doc in source.data)
                    {
                        System.Action closure = () =>
                        {
                            TBasicDoc inner = sample.GetT(doc, source.headers.ToArray());
                            List<string> compositeKey = (
                                compositeUniqueKeyMap.keysColumnsByType.compositeKey);
                ...
                case KeyType.compositeRedundant:
                    ...
                    List<List<string>> keyColumnValues = new List<List<string>>();


                    uniqueInt_byComposite = new Auto_Incr_IntMap<List<string>>(keyColumnValues);
                    compositeRedundantKeyMap = new KeyColumns<TBasicDoc>(
                        sample
                        , KeyType.compositeRedundant
                        , compositeIndexMap: uniqueInt_byComposite);
                    compositeIndexKeyedData = new Dictionary<(List<string>, int), TBasicDoc>();

                    foreach (StringMap doc in source.data)
                    {
                        TBasicDoc inner = sample.GetT(doc, source.headers.ToArray());

                        List<string> compositeKey_isolated =
                            compositeRedundantKeyMap.keysColumnsByType.redundantCompositeKey.Item1;
                        (List<string>, int) compositeIndexedKey =
        ...
        /// <returns></returns>
        public TBasicDoc this[(List<string>, int) compositeIndexedKey]
        {
            get
            {
                TBasicDoc ret;
                if (keyType == KeyType.compositeRedundant)
                {
                    try
                    {
                        compositeIndexKeyedData.TryGetValue(compositeIndexedKey, out ret);
                    }catch (System.Exception err) { throw new System.Exception("Bad TryGetValue: " + err.ToString(), err); }
                    return ret;
                }
                else
                {
