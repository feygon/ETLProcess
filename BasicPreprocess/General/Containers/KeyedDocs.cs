            ...
            switch (keyType)
            {
                case KeyType.primaryUnique:
                    ...
                    foreach (StringMap doc in source.data)
                    {
                        System.Action closure = () =>
                        {
                            ...
                            try
                            {
                                KeyValuePair<string, TBasicDoc> innerKVP = new KeyValuePair<string, TBasicDoc>(
                                    doc[primaryUniqueKeyMap.keysColumnsByType.primaryKey] // value in primarykey column
                                    , inner
                                    );
                                primaryKeyedData.Add(innerKVP.Key, innerKVP.Value);
                            }
                            catch (Exception err)
                            {
                                throw new Exception(String.Format(
                                    "Dereferencing on column {0} or adding referenced key to primaryKeyedData has thrown innerException: "
                                    , primaryUniqueKeyMap.keysColumnsByType.primaryKey), err);
                            }
                        };
                        closure();
                    }
                ...
                case KeyType.primaryRedundant:
                    ...
                    foreach (StringMap doc in source.data)
                    {
                        System.Action closure = () =>
                        {
                            KeyValuePair<(string, int), TBasicDoc> innerKVP = new KeyValuePair<(string, int), TBasicDoc>();
                            TBasicDoc inner = sample.GetT(doc, source.headers.ToArray());
                            string primaryIndexedColumn = primaryRedundantKeyMap.keysColumnsByType.redundandPrimaryKey.Item1;
                            try
                            {
                            // get doc object, add to primary keyed dictionary
                            (string primary, int index) primaryIndexedKey =
                                primaryRedundantKeyMap.keysColumnsByType.GetNextRedundantPrimaryKey(doc[primaryIndexedColumn]);
                                innerKVP = new KeyValuePair<(string, int), TBasicDoc>(
                                        (
                                            primaryIndexedKey.primary
                                            , primaryIndexedKey.index
                                        )
                                        , inner
                                    );
                                primaryIndexKeyedData.Add(innerKVP.Key, innerKVP.Value);
                            }
                            catch (Exception err)
                            {
                                throw new Exception(String.Format(
                                    "Error dereferecing on column {0} or adding key {1} to primaryKeyedData has thrown innerException: "
                                    , primaryIndexedColumn, innerKVP), err);
                            }
                        ...
                        System.Action closure = () =>
                        {
                            ...
                            try
                            {
                                (string primary, string secondary) compositeKey = (
                                    doc[compositeUniqueKeyMap.keysColumnsByType.compositeKey.Item1]
                                    , doc[compositeUniqueKeyMap.keysColumnsByType.compositeKey.Item2]);
                                compositeKeyedData.Add(compositeKey, inner);
                            } catch (Exception err)
                            {
                                throw new Exception(String.Format(
                                    "Error dereferecing on composite key {0} or adding key to primaryKeyedData has thrown innerException: "
                                    , compositeUniqueKeyMap.keysColumnsByType.compositeKey), err);
                            }
                        };
                        closure();
                    }
                ...
                // TO DO: Fix error of adding column name as column entry (which generates already added error)
                ...
                case KeyType.compositeRedundant:
                    ...
                    foreach (StringMap doc in source.data)
                    {
                        ...
                        System.Action closure = () =>
                        {
                            KeyValuePair<(string, string, int), TBasicDoc> innerKVP 
                            = new KeyValuePair<(string, string, int), TBasicDoc>();
                            try
                            {
                                TBasicDoc inner = sample.GetT(doc, source.headers.ToArray());

                                (string, string) compositeKey_isolated = (
                                    doc[compositeRedundantKeyMap.keysColumnsByType.redundantCompositeKey.Item1]
                                    , doc[compositeRedundantKeyMap.keysColumnsByType.redundantCompositeKey.Item2]);
                            ...
                            {
                                throw new Exception(String.Format(
                                    "Adding key {0} to primaryKeyedData has thrown innerException: "
                                    , innerKVP), err);
                            }
                        };
                        closure();
